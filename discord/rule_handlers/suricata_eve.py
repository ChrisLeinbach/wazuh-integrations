...
class SuricataEve(BaseHandler):
    ...
    def generate_fields(self) -> List[dict]:
        sig_field = self.create_new_field("Signature", self.alert_data["data"]["alert"]["signature_id"])
        action_field = self.create_new_field("Action", self.alert_data["data"]["alert"]["action"].capitalize())
        severity_field = self.create_new_field("Severity", self.alert_data["data"]["alert"]["severity"])
        conversation_field = self.create_new_field("Conversation", self.create_conversation_string())
        ...
        if "http" in self.alert_data["data"].keys():
            ...
            alert_data.append(self.create_new_field("HTTP Address", f"{hostname}{url}"))

        if 'tls' in self.alert_data["data"].keys():
            tls_sni = self.alert_data["data"]["tls"]["sni"]
            alert_data.append(self.create_new_field("TLS SNI", f"{tls_sni}"))
        ...
        if 'dns' in self.alert_data["data"].keys():
            ...
            alert_data.append(self.create_new_field("DNS Query", f"{dns_query}"))
